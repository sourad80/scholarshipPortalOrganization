...
class Student(db.Model, UserMixin):
    ...
    applications = db.relationship(
        'scholarship_application', backref='student', lazy=True)
...
class Organization(db.Model, UserMixin):
    ...
    scholarships = db.relationship(
        'Scholarship', backref='organization', lazy=True)
...
class Scholarship(db.Model):
    ...
    applications = db.relationship(
        'scholarship_application', backref='application', lazy=True)
...
class scholarship_application(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    sch_id = db.Column(db.Integer, db.ForeignKey(
        'scholarship.id'), nullable=False)
    ...
    def __repr__(self):
        return f"Scholarship('{self.id}', '{self.status}')"
