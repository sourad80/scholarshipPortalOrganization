...
class AddSch(Form):
    name = StringField('Name', validators = [ DataRequired() , Length(min = 2, max = 25) ])
    description = TextAreaField('Description', validators = [ DataRequired() , Length(min = 2, max = 256) ])
    ...
    cls_xii_min_per = FloatField('Class XII min %',default=0.0)
    cls_ug_min_per = FloatField('UG min %',default=0.0)
    submit = SubmitField('ADD')

    def validate_name(self,name): #Not working
